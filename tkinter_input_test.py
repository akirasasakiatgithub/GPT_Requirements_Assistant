...
def remove_settings(SETTINGS_FILE=SETTINGS_FILE):
    # ファイルが存在するか確認して削除
    if os.path.exists(SETTINGS_FILE):
        try:
...
def split_into_sentences(text):
    """
    日本語の文章を分割してリストにし辞書のdefinitionキーに代入する関数。
    """
...
def show_output_message(title, message, message_type="info"):
    ...
    if message_type == "info":
        msbox = tk.Toplevel()
        msbox.title(title)
        msbox.geometry("400x200")

        label = tk.Label(msbox, text=message, wraplength=300, justify="left", font=("Meiryo", 12))
        label.pack(pady=20)

        button = tk.Button(msbox, text="OK", font=("Meiryo", 12), command=msbox.destroy)
        button.pack(pady=10)
...
def show_messagebox_with_skip(title, message, key):
    ...
    # 引数のmessageをラベルとして追加
    label = tk.Label(msbox, text=message, wraplength=250, justify="left", font=("Meiryo", 12))
    label.pack(pady=10)

    # チェックボックス
    skip_var = tk.BooleanVar() #True（オン）または False（オフ）として状態管理
    skip_checkbox = tk.Checkbutton(msbox, text="次回以降再表示しない", font=("Meiryo", 12), variable=skip_var)
    skip_checkbox.pack(pady=10)

    button = tk.Button(msbox, text="OK", font=("Meiryo", 12), command=on_close)
    button.pack(pady=10)
...
def process_multiline_input(widget, skip_var, has_arg, callback=None):
    """
    ...
    if isinstance(widget, tk.Text):
        user_input = widget.get("1.0", tk.END).strip()
    elif isinstance(widget, tk.Entry):
        user_input = widget.get().strip()
...
def create_gui(callback, argments=None):
    ...
    if argments:
        ...
        text = (f"\n「{argments['sentence']}」\n\nという要求の\n\n”{argments['amb_str']}”\n\nという表現は\n"
                ...
                f"この場合の一先ず明確な回答というのは、例えば\n「{argments['example'][0]}」、\n「{argments['example'][1]}」、\n「{argments['example'][2]}」\nのようなものです。"
                "\n\n※明確にしない場合は「この入力をスキップする」にチェックを入れてください。")
        
        entry_widget = tk.Entry(root, width=140)
        entry_widget.pack(pady=10)

        skip_checkbox = tk.Checkbutton(root, text="この入力をスキップする", font=("Meiryo", 12), variable=skip_var)
        skip_checkbox.pack(pady=10)
        
        input_widget = entry_widget
    else:
        show_output_message(
            "要求定義の入力",
            "複数行の要求定義を入力してください。\n一つの要求を一つの文で入力してください。"
        ...
        text_widget = tk.Text(root, height=30, width=140)
        text_widget.pack(pady=10)
        input_widget = text_widget

    # 説明ラベル
    label = tk.Label(root, text=text, justify="left", font=("Meiryo", 12))
    label.pack(pady=10)
    ...
    process_button = tk.Button(
        ...
        height = 1,
        font=("Meiryo", 12),
        command=lambda: process_multiline_input(input_widget, skip_var, has_arg, callback)
    )
