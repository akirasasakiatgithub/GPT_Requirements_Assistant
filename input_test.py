...
def input_requirement(argments=None):
    ...
    def process_multiline_input(argments=None):
        ...
        if argments:
            print(f"\n「{argments["sentence"]}」という要求の”{argments["amb_str"]}”という表現は{argments["reason"]}という理由で曖昧です。\n{argments["amb_str"]}の内容をより明確、具体的に示して下さい。\n※明確にしない場合は「-（ハイフン）」を入力して下さい。（Enter → Ctrl+Z → Enterで終了））:")
        else:
            print("\n複数行の要求定義を入力してください。\n一つの要求を一つの文で入力してください。（Enter → Ctrl+Z → Enterで終了）:")
